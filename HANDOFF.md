...
| **ビジュアルテーマ** | レベル5ごとに四季と農業テーマが変化。<br>・Lv 1-5: 春の畑（播種・若葉）<br>・Lv 6-10: 夏の水田（清流・水滴）<br>・Lv 11-15: 秋の果樹園（完熟・紅葉）<br>・Lv 16+: 大収穫祭（夜市・星空） |
| **BGM・効果音（SE）** | ・ローカル: `pygame.mixer` を使用して再生。<br>・Web版: ブラウザ制限を回避するため、JavaScriptの Web Audio API（`window._bgmAudio`、`window._playSE`）でBGMおよび全効果音（移動・回転・着地・消去等）を完全合成再生。 |
